...
export const scenarios: Scenario[] = [
  {
    id: 'scenario-01',
    name: 'Prep Kitchen - Station A',
    location: 'Building 2, Floor 1',
    zoneType: 'Food Preparation',
    status: 'safe',
    statusLabel: 'STERILE',
    eventsLast24h: 2,
    eventsCritical: 0,
...
export const alerts: Alert[] = [
  ...
  },
  {
    ...
    timestamp: '10:15:30',
    message: 'Routine sanitization completed successfully',
    status: 'safe',
    isRead: true,
  },
  {
...
export function calculateSystemMetrics(scenarios: Scenario[]): SystemMetrics {
  ...
  const scenariosWarning = scenarios.filter(s => s.status === 'warning').length;
  const scenariosCritical = scenarios.filter(s => s.status === 'critical').length;
  const totalEventsToday = scenarios.reduce((sum, s) => sum + s.eventsLast24h, 0);
  const criticalEventsToday = scenarios.reduce((sum, s) => sum + s.eventsCritical, 0);
